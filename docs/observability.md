...
Route guards use the locally stored session only; they do not issue a separate session-validation request. The API remains the source of truth for every protected request. When the API returns an authentication failure, the frontend clears the session, shows `auth.sessionExpired`, and redirects to login once. Root-provided list stores cache successful list responses by query, share concurrent requests, and keep the last response when a route component is destroyed. Mutations invalidate the affected query; manual refresh is throttled by a 30-second client-side cooldown. When investigating request-count changes, distinguish cache hits, explicit refreshes, mutation-triggered invalidations, and first-load misses in addition to excluding Angular/Vite development module requests and CORS `OPTIONS` preflight requests.
